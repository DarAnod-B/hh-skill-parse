...
def main() -> None:
    ...
    skill_list_ru = [trans.text.lower() for trans in translator.translate(skill_list, src='ru', dest='en')]
    skill_list_count = Counter(skill_list_ru)
    skill_list_count_sort = {skill:count for skill, count in skill_list_count.items() if count > 5}
    skill_list_sort = dict(sorted(skill_list_count_sort.items(), key=lambda item: item[1], reverse=True))
